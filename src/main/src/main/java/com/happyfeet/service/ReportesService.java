...
public class ReportesService {
    ...
    public void NuevaFuncionalidad(LocalDate fechaInicio, LocalDate fechaFin) {
        System.out.println("\n=== 📈 HISTORIAL CLINICO COMPLETO - VETERINARIA HAPPY FEET ===") 

        List<HistorialMedico> historialCompleto = HistorialMedicoDAO.listar();

        if (historialCompleto.isEmpty()) {
            System.out.println("No tenemos los registros para generar el reporte.");
            return;
        }
       
        System.out.println("FECHA DE GENERACION:" + LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
        fechaGeneracion.entrySet().stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed())
                .forEach(entry ->
                        System.out.println("  • " + entry.getKey() + ": " + entry.getValue() + " servicios"));

        System.out.println("\n=== 📈 DATOS DEL PACIENTE ===")
        datosDelPaciente.entrySet().stream()
